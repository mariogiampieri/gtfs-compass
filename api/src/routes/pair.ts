 ...
 *     the caller's IP. Both keys are attacker-chosen with unbounded
 *     cardinality, so each charge reads a *global* counter first and refuses
 *     before writing anything at all — the same ordering `chargeSendBudget()`
 *     uses in `email.ts`, and for the same reason: a cap that is charged after
 *     the write it is supposed to bound, bounds nothing.
 *
 ...
 * whole reason R7 also demands the per-claimer and per-IP budgets below: a
 * per-code counter is no defense at all against spraying the live-code space.
 */
...
/** Starts per IP per UTC day. Attacker-chosen key: bounded by the global slice. */
export const PAIR_START_IP_SCOPE = "pair:start:ip";
/** Starts overall per UTC day — the bound that must be read before an IP row is written. */
export const PAIR_START_GLOBAL_SCOPE = "pair:start:global";
/** Claim attempts per authenticated claimer per UTC day, hits and misses alike. */
export const PAIR_CLAIMER_SCOPE = "pair:claimer";
/** Claim attempts per IP per UTC day — one account per IP is not a bound. */
export const PAIR_CLAIM_IP_SCOPE = "pair:claim:ip";
/** Claim attempts overall per UTC day; read before either keyed counter is written. */
export const PAIR_CLAIM_GLOBAL_SCOPE = "pair:claim:global";

const DEFAULT_START_IP_BUDGET = 20;
const DEFAULT_START_GLOBAL_BUDGET = 500;
const DEFAULT_CLAIMER_BUDGET = 10;
const DEFAULT_CLAIM_IP_BUDGET = 20;
const DEFAULT_CLAIM_GLOBAL_BUDGET = 500;
...
/**
 * The caller's address for the per-IP budgets. `CF-Connecting-IP` is set by the
 * edge and cannot be spoofed by the client; behind anything else every caller
 * shares the `unknown` bucket, which fails safe (one shared, tighter budget)
 * rather than open.
 */
 ...
 * bearing for the same reason: the per-IP key is attacker-chosen with unbounded
 * cardinality (an IPv6 /64 is 2^64 keys), so charging it before the global
 * slice is checked would persist one `auth_budgets` row per address for as long
 * as anyone cares to POST. Reading the global counter first means an exhausted
...
async function chargeStartBudget(env: Env, ip: string): Promise<boolean> {
  const key = await hashToken(ip);
  const globalUsed = await readBudget(env, PAIR_START_GLOBAL_SCOPE, "");
  const ipUsed = await readBudget(env, PAIR_START_IP_SCOPE, key);
  if (globalUsed >= budgetVar(env.PAIR_START_BUDGET_GLOBAL, DEFAULT_START_GLOBAL_BUDGET)) {
    console.warn("[pair] start refused: the global daily start budget is spent");
    return false;
  }
  if (ipUsed >= budgetVar(env.PAIR_START_BUDGET_IP, DEFAULT_START_IP_BUDGET)) return false;
  await incrementBudget(env, PAIR_START_IP_SCOPE, key);
  await incrementBudget(env, PAIR_START_GLOBAL_SCOPE, "");
  return true;
}
 ...
 * claimer and the IP are charged: one account per IP is not a bound, and one IP
 * per account is not either.
 *
 * The claimer key is a SHA-256 of the user id and the IP key a SHA-256 of the
 * address, so `auth_budgets` is not a log of who tried to pair from where.
 */
async function chargeClaimBudget(
  ...
  const ipKey = await hashToken(ip);
  // Every read before any write, global first: see chargeStartBudget.
  const globalUsed = await readBudget(env, PAIR_CLAIM_GLOBAL_SCOPE, "");
  ...
  if (globalUsed >= budgetVar(env.PAIR_CLAIM_BUDGET_GLOBAL, DEFAULT_CLAIM_GLOBAL_BUDGET)) {
    console.warn("[pair] claim refused: the global daily claim budget is spent");
  ...
  await incrementBudget(env, PAIR_CLAIM_GLOBAL_SCOPE, "");
  return { allowed: true };
}
