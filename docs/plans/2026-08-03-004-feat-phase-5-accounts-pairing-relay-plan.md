...
- R20. A Cron Trigger purges `locate_log` rows past `LOCATE_LOG_RETENTION_DAYS` (default 90) in bounded batches, and also sweeps expired `magic_tokens` and `pairing_codes`. The handler records rows-deleted and a completion timestamp; "has not run in N days" is an alert condition.
...
- **Approach:** `POST /v1/auth/request` returns an identical body either way, with the budget check inline and the send in `waitUntil`; a request inside the window resends the existing token. The emailed URL carries the token in the **fragment**; the callback is a **dedicated Worker-served route** (not an SPA route) with a nonce'd CSP and `Referrer-Policy: no-referrer`, whose script reads `location.hash` and POSTs to `/v1/auth/redeem`. Redeem performs the single-use conditional UPDATE, checks rows-affected, **and enforces the custom header plus `Origin`** — without which a hosted page can auto-POST an attacker's token and sign the victim into the attacker's account. A `__Host-` nonce cookie set at request time is matched at redemption; because `SameSite=Lax` means that cookie is absent on a cross-site POST, a mismatch routes to an interstitial that **names the address being signed in**, rather than a bare confirm button.
- **Test scenarios:** unknown vs known address byte-identical with no inline branch (AE1); GET does not consume, later POST succeeds (AE2); cross-site POST without the custom header → 403 (AE3); nonce mismatch shows the naming interstitial and remains redeemable; replay → 400; expired → 400; concurrent double-redeem proves the rows-affected check; budget exhausted → same 200, no send.
...
- **Approach:** a Cron Trigger, not a DO alarm — there is no polling loop here, and the alarm-loop discipline exists for a different shape. Deletes `locate_log` past `LOCATE_LOG_RETENTION_DAYS` (default 90) in bounded batches on the new `ts` index, and sweeps expired `magic_tokens`/`pairing_codes` (nothing else deletes them, and `pair/start` is unauthenticated). Records rows-deleted and completion time.
- **Test scenarios:** aged rows deleted, in-window rows survive (AE11); batching bounds one invocation; expired auth rows swept; a purge with nothing to do is a clean no-op that still records; the counter is observable.
...
- **Is registration open?** With Resend at 100/day, any stranger can create accounts and consume the budget. Does an allowlist gate signup globally, or only guard console mode as currently written? Two implementers would build different things. **Blocks U4.**
...
- **The `locate_log` minimum field set.** Raw `est_*`/`ref_*` coordinates exist only for the residual map — itself optional. Coarse-by-default with an opt-in for precise, and a shorter window for precise rows, costs the map fidelity but turns a metrics table back into metrics rather than movement history. **Blocks U1**, and cannot be walked back cheaply once rows exist.
