...
- R11. A session-authenticated phone POSTs a fix naming one of its own devices; the device fetches its current fix with its own token. **The two sides resolve identity differently and both must be exact:** a session resolves to a *user*, who may own several devices, so the phone's request carries a `device_id`, the resolver rejects with 403 unless that row's `user_id` matches the session, and the relay row is keyed by the **validated `devices.id`** — never by the raw body value. The device's read side is purely credential-derived, since its token identifies exactly one device. This is the plan's primary IDOR surface. With no fix present the device falls through to the existing chain, and the acceptance test is that **the device works normally with the phone powered off**.
...
- AE10. **Given** an account deletion, **then** rows in all dependent tables are gone, including the device's relay row.
...
- **Approach:** a `device_fixes` table keyed by `devices.id` with `ON DELETE CASCADE`, one row per device, holding `{lat, lon, accuracyM, capturedAt, receivedAt}` — deliberately **not** columns on `devices`, whose row is on the auth hot path. Three exported functions and nothing else: `putFix` (upsert, latest-wins, **with one refinement:** a newly-arrived fix does not replace a strictly more accurate one still inside the 120 s horizon, so a momentary coarse reading can't erase a good position the device is about to read), `getFix`, `clearFix`. Callers never write SQL against the table directly — that discipline is what makes the documented DO upgrade a two-function change.
- **Patterns to follow:** the seam shape of `locate.ts` and `walk.ts` — exported functions, no class, policy in one place.
- **Test scenarios:** put→get round-trip; latest-wins under rapid puts; a coarse fix does not displace a fresher, more accurate one inside the horizon; absence is distinct from staleness; a fix past the horizon reads as last-known-with-age, not as a position; `clearFix` removes the row; two device ids never share state; deleting a device cascades its fix away.
...
- **Which device does the phone relay to?** Once R11's ownership check is in place, a user with two paired devices still needs a rule: relay to a selected device, to a per-account default, or fan out to all of them. Fan-out multiplies the relay write volume. **Shapes U14** and the M3 UI.
