...
- R3. Sessions are opaque 128-bit tokens stored hashed in D1, delivered as `__Host-`prefixed `Secure; HttpOnly; SameSite=Lax; Path=/` cookies, rotated on authentication, 30-day sliding renewed at half-life so reads don't write, 180-day absolute. **Every state-changing route whose credential is ambient — the session cookie, and the pre-session `__Host-` nonce cookie that `/v1/auth/redeem` matches — requires a custom request header and an `Origin` check; an absent `Origin` is a 403.** The rule is deliberately scoped to ambient credentials, because that is the entirety of CSRF's threat model: `POST /v1/device/pair/start`, `POST /v1/device/pair/poll`, and the `DIAG_TOKEN` branch of `/v1/locate/ref` authenticate with a Bearer credential a browser never attaches on its own, and they are called by harnesses and firmware that send no `Origin` header at all. A blanket rule would 403 the entire pairing flow — M2's whole outcome — while protecting nothing.
...
- R5. `AUTH_MODE=single` skips login and binds everything to a fixed synthetic user. It is an auth bypass: exact-string match only, anything unset or unrecognized fails closed to multi-user, CSRF and `Origin` checks still enforced, and the PWA shows a persistent banner. **Those controls constrain browsers only, and must be documented as such** — with no credential in play, any non-browser client that reaches the Worker hostname can read the account's named origins (home and office coordinates), post relay fixes into its devices, and delete the account. Workers deploy to a public hostname by default, so `AUTH_MODE=single` is only safe behind a network-level control (Cloudflare Access, a private hostname, or equivalent). The README, `.env.example`, and the PWA banner all say so.
...
- R11. **The phone posts its position for the account, not for a device** (Mario, 2026-08-04): the request names no device, the server writes the fix to every one of that user's devices **holding the `read:fix` grant and not revoked** (`revoked_at IS NULL` is part of the fan-out predicate, or the next post silently re-creates the row unpair just cleared), and each device reads its own row with its own token. Naming a device would have created the plan's primary IDOR surface for no benefit — with an account-scoped post there is nothing to spoof, since the session identifies the user and the grant list identifies the recipients, and the phone needs no device-selector UI. **The post is session-authenticated only**, and carries a per-session and per-IP D1 write budget like every other write this phase adds: it is the highest-frequency write path in the system and fans out to N rows per call, so leaving it as the one budget-free path would let a single scripted client saturate the same single-primary database that serves session validation. With no fix present the device falls through to the existing chain, and the acceptance test is that **the device works normally with the phone powered off**.
...
- R13. Freshness is explicit and modeled on Garmin's `Position.Quality`: a last-known fix is a distinct state, not a fix with an old timestamp. **That state needs somewhere to live on the wire** — today's `ResolvedLocation` is a binary `{known:true, …} | {known:false}` union with no age field, so the locate response gains `captured_at` and a `quality: "current" | "last_known"` discriminator. The device renders relative age and provider ("via phone, 3 min ago"); absent or gated location renders `{"known": false}`, never a stale number.
- R14. Relay state is latest-wins, never queued — a stale queued fix is worse than none. It lives in a dedicated `device_fixes` table keyed by `devices.id`, one row per device, behind a `putFixForUser`/`getFix`/`clearFix` seam so the storage choice stays swappable.
...
- R18b. **Origin selection follows the spec: coarse location picks it.** Authenticated `/v1/departures` reuses the `lat`/`lon`/`acc` parameters the endpoint already accepts — the server matches that position to the nearest named origin and uses its `walk_times` row as the manual tier. **The match is gated and bounded, not merely nearest.** A supplied fix coarser than `LOCATE_MAX_ACCURACY_M` counts as location-unknown for this purpose (today only `computeWalk` gates on accuracy; the route layer accepts any positive `acc`, so an unchecked match would let a 3 km fix select an origin confidently), and the nearest origin must lie within a new `ORIGIN_MATCH_MAX_M` (default 750) — beyond which nothing matches and walk falls through to the heuristic tier computed from the supplied position, rather than inheriting home's stored walk times from 40 km away. **When location is unknown and more than one origin exists, the response carries arrivals with no leave-by** (Mario, 2026-08-04): the same discipline as the empty-`m` dash and the negative `l` — a value that cannot be computed is absent, never guessed. With exactly one origin, that origin is unambiguous and is used.
...
- R20. A Cron Trigger purges `locate_log` on a **two-tier schedule** (Mario, 2026-08-03): raw coordinate columns (`est_lat`/`est_lon`/`ref_lat`/`ref_lon`) are nulled after `LOCATE_LOG_PRECISE_DAYS` (default 14) while the derived metrics that answer the actual question — `delta_m`, accuracies, `bssid_count`, `provider`, `ts` — survive to `LOCATE_LOG_RETENTION_DAYS` (default 90), after which the row is deleted. The residual map keeps full fidelity on recent data; the movement history ages out fast. The same trigger sweeps expired `magic_tokens`, `pairing_codes`, **and `device_fixes` rows whose `captured_at` is older than `LOCATE_LOG_PRECISE_DAYS`** — a live GPS fix at metre accuracy is the most precise location this system stores, and it would otherwise be the only location table with no expiry at all, retained indefinitely for a user who simply stopped using the relay while a 14-day-old 500 m estimate gets nulled. The handler records rows affected and a completion timestamp; "has not run in N days" is an alert condition.
- R21. One-click "delete all my location history" — which **clears the user's `device_fixes` rows** as well as their `locate_log` rows, since the relay row is location history by any reading — and account deletion that verifiably cascades, including the relay row, which the `device_fixes` FK cascade covers for free. This requires attribution to exist in the first place: authenticated locate paths populate `locate_log.user_id` and `device_row_id` from the resolved credential (both stay NULL on the anonymous path), because every insert in the codebase today hardcodes `user_id` to NULL and the existing cascade therefore reaches zero rows.
...
- Hibernatable-WebSocket relay delivery; polling ships, and the relay seam stays RPC-shaped so the swap is a handler-plus-firmware change, never a data-model migration.
...
- AE6c. **Given** two named origins and a request with no resolvable location, **then** the response carries arrivals and no leave-by values; **given** exactly one origin, **then** its walk times are used; **given** a position 40 km from every origin, **then** none matches and walk falls back to the heuristic.
- AE6d. **Given** a device that has a stored fix and whose `read:fix` grant is then revoked, **then** its very next `/v1/locate` and `/v1/nearby` calls resolve without the phone provider, and a subsequent phone post writes it no new row.
...
- **One phone-fix ingress, not two — and it does not do what its name suggests.** `POST /v1/locate/ref` today does **not** insert a row: it `UPDATE`s the newest *unpaired* `locate_log` estimate for a device within a 60-second window and returns 404 when none exists (row insertion lives on `POST /v1/locate` behind `log: true`). Rather than adding a second, divergently-authenticated endpoint, the relay extends `/v1/locate/ref`: **`relay: true` requires a session, calls `putFixForUser` (writing `device_fixes`), and short-circuits the pairing entirely**, returning 200 whether or not an unpaired estimate exists; `log: true` accepts a session or `DIAG_TOKEN` and keeps today's pairing behavior, including its 404. Without that short-circuit, a phone posting a relay fix with no device WiFi scan in the last 60 seconds — the normal case — would fall into the 404 branch. **The two flags authenticate differently on purpose:** account-scoping means the write function takes a *user*, and `DIAG_TOKEN` is a static operator secret naming no user at all — so an operator relay post would have to supply a `user_id` parameter, reinstating exactly the caller-supplied ownership argument R11 exists to delete, now behind a shared secret. `DIAG_TOKEN` stays on the diagnostic path where it belongs.
...
- **Approach:** mirror `locate.ts`'s seam shape — exported types, pure functions, policy in exactly one place, no classes. Resolves a request to `{kind:"session", userId} | {kind:"device", deviceId, userId, scopes} | null`, and **returns the ownership predicate that data routes must apply** rather than trusting each route to remember — the reason being that today's `locate_log` read is an unscoped `SELECT *`. Session mint/validate/slide/rotate/revoke per R3; CSRF header plus `Origin` check with absent-`Origin` rejection, **applied to ambient-credential requests only** (session cookie and the redeem nonce cookie) so the Bearer-authenticated pairing and diagnostic routes are not 403'd for sending no `Origin`; `AUTH_MODE=single` short-circuit with fail-closed parsing.
- **Execution note:** implement the resolver test-first — every later unit trusts it, and its denials are the security contract.
- **Test scenarios:** session round-trip and expiry; sliding renewal writes only past half-life; rotation invalidates the prior token; missing custom header → 403; cross-origin and **absent** `Origin` → 403 on a cookie-authenticated route; a Bearer-authenticated route with no `Origin` succeeds (the pairing-flow regression); `AUTH_MODE=single` yields the synthetic user; unrecognized `AUTH_MODE` fails closed to multi-user; a device credential never resolves to a session kind.
...
- **Approach:** a Cron Trigger, not a DO alarm — there is no polling loop here, and the alarm-loop discipline exists for a different shape. Runs R20's two tiers in bounded batches on the `ts` index: null the raw coordinate columns past `LOCATE_LOG_PRECISE_DAYS` (14), delete rows past `LOCATE_LOG_RETENTION_DAYS` (90). Sweeps expired `magic_tokens`/`pairing_codes` (nothing else deletes them, and `pair/start` is unauthenticated) and `device_fixes` rows past `LOCATE_LOG_PRECISE_DAYS` (R20 — otherwise the most precise location in the system is the only one that never expires). Records rows affected and completion time.
- **Test scenarios:** rows past 14 days keep their metrics but lose raw coordinates; rows past 90 days are deleted; in-window rows are untouched (AE11); a `device_fixes` row past the precise window is deleted while a recent one survives; a row that already had its coordinates nulled is not re-processed; batching bounds one invocation; expired auth rows swept; a purge with nothing to do is a clean no-op that still records; the counters are observable.
...
- **Test scenarios:** a test per denied route (email, device list, config write, pair claim) → 403 (AE6); a `read:departures`-only token reading a fix → 403; **a device token reading `/v1/config/:device_id` for a device id that is not its own → 403** (U15's route is the one place a client-supplied device id survived the relay redesign); revoked → 401; unknown → 401; query-param token rejected; `last_used_at` slides without per-request writes.
...
- **Approach:** the code-entry field and confirm screen from R8, plus a device list showing name, last-seen, firmware version, per-scope toggles, and unpair (revoke). Session-authenticated, scoped to the owning user by the resolver's predicate. The `read:fix` toggle is the privacy control *and* the relay's fan-out control (R9/R11), so the UI should say what granting it means in plain language — this device will receive your phone's live position — rather than presenting three interchangeable checkboxes. **Toggling `read:fix` off calls `clearFix(deviceId)` exactly as unpair does**, so revocation removes the position already delivered rather than only stopping the next one.
- **Test scenarios:** claim flow end to end from the UI's endpoint; a freshly claimed device shows `read:fix` off; another user's devices invisible and unmodifiable; unpair revokes and a subsequent device call → 401; scope toggle takes effect on the next device request; revoking `read:fix` clears the stored fix, stops future fan-out, and the device's next `/v1/locate` and `/v1/nearby` resolve without the phone provider (AE6d); device-supplied name renders escaped.
...
- **Approach:** a `device_fixes` table keyed by `devices.id` with `ON DELETE CASCADE`, one row per device, holding `{lat, lon, accuracyM, capturedAt, receivedAt}` — deliberately **not** columns on `devices`, whose row is on the auth hot path. Three exported functions and nothing else: `putFixForUser(userId, fix)` (writes one row per device of that user holding `read:fix` **and `revoked_at IS NULL`** — without the second half of the predicate an unpaired device whose scopes row still lists the grant has its fix re-created on the next post, quietly undoing the `clearFix` U16 performs, and U16's "unpair clears the fix immediately" scenario passes while the property it asserts does not hold; latest-wins, **with one refinement:** a newly-arrived fix does not replace a strictly more accurate one still inside the 120 s horizon, so a momentary coarse reading can't erase a good position the device is about to read), `getFix(deviceId)`, `clearFix`. The write function takes a *user*, never a device — that signature is what makes R11's account-scoped post structurally incapable of targeting someone else's device. Callers never write SQL against the table directly; that discipline is what keeps the documented DO upgrade a three-function change.
- **Patterns to follow:** the seam shape of `locate.ts` and `walk.ts` — exported functions, no class, policy in one place.
- **Test scenarios:** put→get round-trip; a user with two devices where one lacks `read:fix` writes exactly one row (AE6b); a user with no granting devices writes nothing and does not error; a revoked device holding the grant receives no row (AE6d); latest-wins under rapid puts; a coarse fix does not displace a fresher, more accurate one inside the horizon; absence is distinct from staleness; a fix past the horizon reads as last-known-with-age, not as a position; `clearFix` removes the row; two device ids never share state; deleting a device cascades its fix away.
...
- **Approach:** extend the existing `/v1/locate/ref` per the single-ingress KTD. `relay: true` **requires a session** (it has no user to write for otherwise), calls `putFixForUser` **ungated** (the gate is read-side, R12), and short-circuits the 60-second pairing lookup so it returns 200 with no unpaired estimate present; `log: true` accepts a session or `DIAG_TOKEN` and keeps today's pairing-and-404 behavior. **The request names no device** (R11) — the session identifies the user, the grant list identifies the recipients, so there is no ownership parameter to validate and no device-selector UI. The relay post carries a per-session and per-IP D1 budget in U1's sharded-counter shape (R11); it is the phase's highest-frequency write and the only one that would otherwise be uncapped. **Attribution lands here, which means giving the insert path a credential:** `POST /v1/locate` with `log: true` accepts a session or paired-device credential alongside `DIAG_TOKEN` and populates `locate_log.user_id`/`device_row_id` from it, keying the daily cap on `devices.id` for authenticated inserts while anonymous inserts keep today's free-form `device_id` cap. Without that, no row is ever created carrying a `user_id` — `/v1/locate/ref` only UPDATEs — and R21's history deletion has nothing to delete. Batching follows the wearable convention — no more than about once a minute, flush on foreground.
- **Test scenarios:** a session-authenticated post writes fixes for the user's granting devices and returns 200 with no unpaired estimate present; the same post writes nothing for another user's devices (structurally, since the seam takes a user); posting without the CSRF header → 403; **`relay: true` with `DIAG_TOKEN` and no session → 401**; posts above the budgeted cadence are refused; `relay` and `log` are independent; the DIAG_TOKEN path retains today's pairing-update behavior including its 404 (regression); a coarse fix is stored (gating is read-side) and still pairs as a reference under `log`; authenticated rows carry `user_id` and `device_row_id` while anonymous rows keep both NULL.
- **Verification:** locate suite green; a real fix posted from a phone lands in `device_fixes`.
...
- **Approach:** split `resolveFromWifi` (keeps the BSSID-hash cache and negative-caching, now returning an **ungated** outcome) from an ordered `resolveLocation(ctx)` composing phone → wifi → unknown. Gate **each** provider outcome inside the loop with reject-and-continue, using the single exported gate function. Apply at the shared seam so `/v1/nearby` inherits it. **The phone provider is skipped unless the resolved credential currently carries `read:fix`** (R9) — the grant governs the read as well as the fan-out, or a device whose grant was just revoked keeps serving the row already written to it. **The last-known state needs wire representation:** `ResolvedLocation` and the `/v1/locate` response gain `captured_at` and a `quality: "current" | "last_known"` discriminator (R13), since the shipped type is a binary known/unknown union with no age field and the device cannot render "via phone, 3 min ago" without it; a fix past the 120 s horizon falls through to WiFi first and is returned as `last_known` only when no fresher provider resolves. That is a device-facing wire change and lands in M3, where the plan's stop condition already contemplates one. The empty-BSSID short circuit moves below the phone provider so a device that skips its radio scan can still be located. Existing direct `resolveLocation(bssids, env)` call sites in the locate suite split between the two functions — the gate-behavior tests belong to the composer.
- **Test scenarios:** fresh accurate fix wins and reports provider/age; a 900 m fix is skipped **and BeaconDB is consulted** (AE7 — the regression the current control flow would fail); a **cached** coarse WiFi fix is still rejected on a cache hit (the consequence of moving the gate above the cache); two devices with identical BSSIDs never share a phone-derived position (AE8); no fix ever posted → chain byte-identical to today (AE9); a device without the `read:fix` grant never consults the relay even with a row present, and a device whose grant is revoked stops on its very next call (AE6d); a fix past the 120 s horizon falls through and surfaces as `last_known` with its age only when nothing fresher resolves; zero-BSSID request with a phone fix resolves; `/v1/nearby` uses the same resolution as `/v1/locate` for the same device; the gate value comes from the same env var as every other provider.
...
- **Files:** `config-ui/`, `api/src/routes/config.ts`, `api/test/workers/config.test.ts`.
- **Approach:** session-authenticated CRUD scoped by the resolver's predicate, bumping `users.config_version` on every write; favorites carry `sort_order`; walk times are per (favorite, origin) and display `source` so estimates look different from confirmed values. `GET /v1/config/:device_id` serves a device its own config by token with an ETag of `{users.config_version}.{feeds.data_version}` — both counters land in U1, and the feed half is required or a recolored route serves stale forever. **The config is resolved from the credential-derived device, and a path segment that does not match it is a 403** — this route is the one place a client-supplied device id survived the relay redesign, and keying the lookup off the path would let one owner's board read another account's favorites and named origins.
- **Test scenarios:** CRUD round-trips scoped to the owner; another user's rows invisible; a device token presenting another device's id in the path → 403; ETag returns 304 unchanged and changes when either version moves; ordering persists; a device token reads config and cannot write it.
...
- **Approach:** `routeDepartures` resolves `{refs, walkSeconds, origin}` from **either** the query string or D1 and hands `composeDepartures` the identical shape it takes today — `departures.ts` and `walk.ts` stay auth-unaware. Origin selection per R18b: the existing `lat`/`lon`/`acc` params match to the nearest named origin, whose `walk_times` row becomes the manual tier — gated at `LOCATE_MAX_ACCURACY_M` and bounded by `ORIGIN_MATCH_MAX_M` (both added to `.env.example`), so a coarse or far-away position matches nothing rather than confidently matching home. With no resolvable location and more than one origin, the response carries no leave-by rather than a guess. **`walk_times.source` travels through to `w[key].src` rather than being derived from the tier**, and the response union widens to the stored vocabulary (`manual | heuristic | mapbox`) — today `composeDepartures` labels the manual tier `"manual"` unconditionally, which would render a server-side estimate as a confirmed value and erase the exact distinction `walk.ts`'s `source` field exists to preserve. That widening is device-facing and needs Mario's sign-off under the stop condition. Note the key shapes differ: `walk_times` is keyed by `favorite_id` while `walkSeconds` is keyed by `"feed:stop"`, and `favorites.stop_ids` is a JSON array — one row fans out to every stop ref of its favorite. **Over-cap expansion truncates, it does not reject:** favorites expand in `sort_order` and stop at `MAX_STOP_REFS`, because the anonymous path's 400 would brick a working device the moment a user adds one favorite too many.
- **Test scenarios:** authenticated mode returns the same shape as anonymous for equivalent input; anonymous query-string mode byte-identical to today (regression); a position near the home origin selects home's walk times and a position near the office selects the office's (the spec's own acceptance criterion); unknown location with two origins returns arrivals and no leave-by, with one origin returns that origin's times (AE6c); a fix coarser than the accuracy gate, and a position beyond `ORIGIN_MATCH_MAX_M`, both match no origin and fall through to the heuristic (AE6c); walk_times rows act as the manual tier for authenticated callers while params still do for anonymous; a `heuristic`-sourced walk_times row is labeled `heuristic` on the wire, not `manual`; over-cap favorites truncate in `sort_order` rather than 400; warm latency stays inside the spec's 300 ms criterion.
...
- **Approach:** one-click history deletion and account deletion as authenticated routes, relying on FK cascades — tested explicitly, because D1's `PRAGMA foreign_keys` behavior is unverified in this repo. `device_fixes` cascades from `devices`, so the relay row needs no special handling on account deletion; unpair and `read:fix` revocation both call `clearFix` directly so a revoked device stops exposing a position immediately rather than at its next overwrite — which holds only because U7's fan-out predicate excludes revoked devices, or the next post re-creates the row. **History deletion clears `device_fixes` too** (R21): the relay row is location history by any reading, and leaving it behind would make "delete all my location history" untrue about the most precise position in the system.
- **Test scenarios:** account deletion empties all dependent tables including `device_fixes` (AE10); the deleted user's attributed `locate_log` rows are gone while legacy anonymous rows survive for the retention purge to handle (the test that would pass vacuously without U14's attribution); history deletion removes only the requesting user's rows — `locate_log` and `device_fixes` both — and leaves devices paired; unpair clears the fix immediately and a subsequent phone post does not restore it; deletion is idempotent.
...
- U7's refinement that a new fix cannot displace a strictly more accurate one inside the 120 s horizon means a *walking* user whose GPS accuracy degrades keeps an older position for up to two minutes. Age is reported honestly so nothing is mislabeled, but confirm the rule is meant for the momentary-coarse-reading case rather than sustained movement — a "not more than 30 s older" clause would cover both.
- Whether revoking `read:fix` should also revoke the device token outright, on the theory that a device whose tracking grant is withdrawn is often a device the owner no longer trusts.
- Whether `device_fixes` should store reduced precision past the freshness horizon, given that the row's only post-horizon use is R13's last-known-with-age rendering.
...
- Cost-cliff guards in place: no non-hibernatable socket exists anywhere, and no SQL touches `device_fixes` outside the relay seam (the property that keeps the documented DO upgrade a three-function change).
